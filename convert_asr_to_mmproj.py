...
-----
    python convert_asr_to_mmproj.py \
        ...
        --quant     Q8_0 \
        -o          /path/to/mmproj-asr-multilingual-Q8_0.gguf
...
def main() -> None:
    ...
    ap.add_argument("--container", type=Path, default=None,
                    help="nemotron_voicechat_11b-*.gguf, for proj and the mel featurizer. "
                         "Not needed when --asr-dir already carries them, which is what "
                         "align_asr.py writes")
    ...
    ap.add_argument("--encoder-state", type=Path, default=None,
                    help=".npz of HF-named encoder tensors from asr_align.fuse, used "
                         "instead of the safetensors; config.json is still read from "
                         "--asr-dir")
    ...
    # Where proj and the mel featurizer come from. A directory written by
    # align_asr.py carries both, and then this converter needs nothing else --
    # no 12 GB container, no torch, no alignment file on the side. A published
        ...
        for name in ("proj.bias", "preprocessor.featurizer.fb", "preprocessor.featurizer.window"):
            if name not in st:
                raise SystemExit(
                    ...
                    "export. Re-run align_asr.py, or pass --container."
                )
    else:
        if args.container is None:
            raise SystemExit(
                f"{args.asr_dir} carries no proj, so --container is needed for it and "
                "for the mel featurizer. align_asr.py writes a directory that carries "
                "both."
            )
        logger.info("container : %s (proj and the mel featurizer)", args.container)
        src = GGUFSource(args.container)

    # Every encoder tensor goes through `enc`, so --encoder-state can put a fused
    # set of weights in the safetensors' place without touching the writing
    # below. The names are the same either way: asr_align keeps the HF port's
    # naming for exactly this reason.
    if args.encoder_state is not None:
        fused = np.load(args.encoder_state)
        available = set(fused.files)

        def enc(name: str) -> np.ndarray:
            if name not in available:
                raise SystemExit(f"{args.encoder_state}: no tensor named {name}")
            return np.asarray(fused[name], dtype=np.float32)

        logger.info("encoder   : %s (fused, %d tensors)", args.encoder_state, len(available))
    else:
        enc = st.f32
